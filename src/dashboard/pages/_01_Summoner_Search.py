import json
import streamlit as st
from src.dashboard.utils.riot import lookup_summoner
from src.dashboard.data.stats import display_general_stats, display_champion_stats


def main():
    st.set_page_config(page_title="Summoner Search", layout="wide")
    st.title("Summoner Search  ↪")

    # Inputs de búsqueda
    name_input = st.text_input(
        "Enter Summoner Name or Riot ID (Name#TAG)",
        value="",
        placeholder="p. ej. MEMENTO MØRI#提莫国王"
    )
    region = st.text_input(
        "Región (platform tag)",
        value="euw1",
        help="euw1, na1, kr, etc."
    )

    # Botón de búsqueda
    if st.button("Search"):
        if not name_input.strip():
            st.warning("Escribe un nombre o Riot ID.")
            st.stop()

        # Llamada a Riot API
        try:
            summ = lookup_summoner(name_input.strip(), region.strip().lower())
            st.session_state["summoner"] = summ
            st.success("¡Invocador encontrado y guardado en la sesión!")
        except Exception as e:
            st.error(f"No se pudo obtener el invocador: {e}")
            st.stop()

    # Obtener el invocador de la sesión
    summ = st.session_state.get("summoner")
    if not summ:
        st.info("Primero busca un invocador; quedará guardado para las demás pestañas.")
        st.stop()

    # Mostrar detalles básicos del invocador
    c1, c2, c3 = st.columns([2, 1, 2])
    with c1:
        st.subheader("Name")
        st.write(summ.get("name") or "—")
    with c2:
        st.subheader("Level")
        st.metric(label="Level", value=summ.get("level") or "—")
    with c3:
        st.subheader("P UUID / ID (encryptedSummonerId)")
        st.text_input("PUUID", value=summ.get("puuid") or "—", disabled=True)
        st.text_input("ID (encryptedSummonerId)", value=summ.get("id") or "—", disabled=True)

    # Mostrar JSON completo
    with st.expander("Ver JSON completo", expanded=False):
        st.code(json.dumps(summ, ensure_ascii=False, indent=2))

    # Mostrar estadísticas generales
    display_general_stats(summ.get("puuid"), region.strip().lower())

    # Mostrar estadísticas por campeón
    display_champion_stats(summ.get("puuid"), region.strip().lower())

    # Estilo para las tablas
    st.markdown("""
        <style>
            .dataframe {
                border-radius: 10px;
                border: 1px solid #ddd;
                padding: 10px;
                background-color: #f5f5f5;
            }
            .dataframe th {
                background-color: #005A9C;
                color: white;
                font-weight: bold;
            }
            .dataframe td {
                color: #333;
                padding: 10px;
            }
        </style>
    """, unsafe_allow_html=True)


# Ejecutar solo si este archivo es la página principal
if __name__ == "__main__":
    main()
